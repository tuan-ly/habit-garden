...
Sanctuary terrain uses **Hybrid Procedural Terrain**: `ground-plane-geometry.ts` owns deterministic responsive face samples and shared visual height, while `GroundPlaneCanvas` clips decoded, lighting-neutral soil/edge materials into those faces. The cinematic bank uses `0.32 × tileSize` side depth and `0.46 × tileSize` front depth so the earth mass stays subordinate to the plantable surface; Stats Garden does not use this path. Both soil faces share one continuous texture sample and a smooth directional-light overlay. Their shared front-bottom point remains a controlled rounded cusp, connected to the center cap by a softly feathered asymmetric crease, so the planes visibly intersect without a hard mechanical seam. Material assets must trigger at most one static offscreen redraw after decode, and pan/zoom/focus must only composite or transform existing output.
...
Development asset review uses **Asset Calibration Studio** at `/dev/asset-studio`. It loads generated analysis plus `config/game-asset-overrides.json`, previews canonical assets and temporary PNG imports against sanctuary/edge/mobile contexts, and atomically regenerates both manifests after a validated save/reset. The route/API return 404 outside development and never use Supabase. The source of truth is `auto analysis → reviewed partial override → defaults`; do not edit generated manifests directly or restore component-level scale magic numbers.
