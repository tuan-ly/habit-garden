...
-- Atomic craft_decoration: deduct materials + award decoration in single tx
...
BEGIN
  ...
  LOOP
    ...
    WHERE user_id = p_user_id
      ...
      AND material_id = v_ingredient.material_id
    FOR UPDATE;
  ...
  -- Upsert decoration into inventory
  ...
  ON CONFLICT (user_id, item_type, decoration_type_id)
    WHERE decoration_type_id IS NOT NULL
...
REVOKE ALL ON FUNCTION public.craft_decoration(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.craft_decoration(uuid, uuid) TO authenticated, service_role;


-- Atomic purchase_decoration: spend coins + award decoration in single tx
...
BEGIN
  ...
  WHERE id = p_decoration_type_id;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'decoration_not_found';
  END IF;
  IF v_deco.coin_price IS NULL THEN
    RAISE EXCEPTION 'not_for_sale';
  END IF;

  SELECT level, subscription_tier INTO v_profile
    FROM public.profiles WHERE id = p_user_id FOR UPDATE;
  IF NOT FOUND THEN
    RAISE EXCEPTION 'profile_not_found';
  END IF;
  IF v_profile.level < v_deco.unlock_level THEN
  ...
  IF v_user_tier < v_req_tier THEN
    RAISE EXCEPTION 'tier_required:%', v_deco.subscription_tier;
  END IF;

  -- Deduct coins atomically (lock profile row above)
  SELECT coins INTO v_balance FROM public.profiles WHERE id = p_user_id;
  IF v_balance < v_deco.coin_price THEN
    RAISE EXCEPTION 'insufficient_coins';
  END IF;

  UPDATE public.profiles SET coins = coins - v_deco.coin_price, updated_at = now()
    WHERE id = p_user_id;
...
REVOKE ALL ON FUNCTION public.purchase_decoration(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.purchase_decoration(uuid, uuid) TO authenticated, service_role;


-- Atomic pickup_decoration: remove from world + add to inventory
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'decoration_not_found';
  END IF;
  IF v_deco.user_id <> p_user_id THEN
...
REVOKE ALL ON FUNCTION public.pickup_decoration(uuid, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.pickup_decoration(uuid, uuid) TO authenticated, service_role;
;
