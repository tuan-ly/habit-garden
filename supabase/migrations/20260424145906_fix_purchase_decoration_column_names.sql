...
-- Fix purchase_decoration function: uses wrong column names
-- transaction_type → reason, source_id → reference_id
-- to match actual coin_transactions table schema
...
BEGIN
  ...
  INSERT INTO public.coin_transactions (user_id, amount, reason, reference_id, balance_after)
    VALUES (p_user_id, -v_deco.coin_price, 'purchase_decoration', p_decoration_type_id::text, v_balance - v_deco.coin_price);
