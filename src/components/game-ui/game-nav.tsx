...
export function GameNav({ user }: GameNavProps) {
  ...
  return (
    <>
      ...
      <nav className="fixed bottom-0 left-0 right-0 z-50 px-2 pb-2 pointer-events-none">
        <div className="max-w-lg mx-auto pointer-events-auto">
          {/* Glowing background effect */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/20 to-transparent rounded-2xl blur-xl" />

          {/* Main nav container */}
          <div className="relative bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-2xl border border-white/20 dark:border-slate-700/50 shadow-2xl shadow-black/20">
            <div className="flex items-center justify-around px-2 py-1">
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.title}
                    href={item.url}
                    className="relative group flex flex-col items-center py-2 px-3 transition-all duration-300"
                  >
                    ...
                    <div className={cn(
                      ...
                      isActive
                        ? `bg-gradient-to-br ${item.color} shadow-lg ${item.glowColor} scale-110 -translate-y-1`
                      ...
                      {isActive && (
                        <>
                          ...
                          <div className="absolute inset-0 rounded-xl animate-ping bg-white/20 pointer-events-none" style={{ animationDuration: '2s' }} />
                        </>
                      )}
                    </div>
                    ...
                    {/* Label */}
                    <span className={cn(
                      "text-[10px] font-semibold mt-1 transition-all duration-300 uppercase tracking-wider",
                      isActive
              ...
              <button
                ...
                className="relative group flex flex-col items-center py-2 px-3 transition-all duration-300"
              >
                <div className="relative flex items-center justify-center w-12 h-12 rounded-xl bg-slate-100 dark:bg-slate-800 group-hover:bg-slate-200 dark:group-hover:bg-slate-700 transition-all duration-300">
                  <Menu className="w-6 h-6 text-slate-500 dark:text-slate-400 group-hover:text-slate-700 dark:group-hover:text-slate-200 transition-colors" />
                </div>
                <span className="text-[10px] font-semibold mt-1 text-slate-400 dark:text-slate-500 uppercase tracking-wider">
                  Menu
                </span>
              </button>
            </div>
