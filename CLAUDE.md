...
| `develop` | Integration branch, latest features | master | master (via release) |
| `feature/*` | One feature per branch | develop | develop |
| `hotfix/*` | Urgent production fixes | master | master + develop |
| `release/*` | Release prep, bug fixes only | develop | master + develop |

### Rules

1. **Never commit directly to master.** Always merge from release or hotfix.
2. **Never commit directly to develop.** Always merge from feature branches.
3. **Feature branches are short-lived.** Merge into develop when feature is done, then delete.
4. **Tag every release** on master: `v1.0.0`, `v1.1.0`, `v1.1.1` (semver).
